...
function querySelectedRenderProperties(string) {
  ...
  var selector = query.select(string)
  selector.length = 1
  console.log(selector)
	if (selector.length === 0) {
		...
		for (var i = 0; i < selector.length; i++) {
			returnArray.push(getRenderProperties(selector));
		}
...
var CanvasRenderer = function () {
	...
	_createClass(CanvasRenderer, [{
      ...
      // if (!encodingOptions) {
			...
			// }
      // this.prepareCanvas();
      (0, _shared.calculateEncodingAttributes)(this.encodings, this.options, this.options.ctx);
      ...
      options.width = options.width / encoding.data.length
